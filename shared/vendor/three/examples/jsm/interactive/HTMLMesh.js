...
const _mouseToPointer = {
	mousedown: 'pointerdown',
	mousemove: 'pointermove',
	mouseup: 'pointerup',
};

function dispatchPointerEquivalent( target, event, init ) {

	const pointerType = _mouseToPointer[ event ];
	if ( ! pointerType || typeof PointerEvent === 'undefined' ) return;
...
function htmlevent( element, event, x, y ) {
	...
	dispatchPointerEquivalent( window, event, mouseEventInit );
	...
	function traverse( element ) {
		...
		if ( element.nodeType !== Node.TEXT_NODE && element.nodeType !== Node.COMMENT_NODE ) {
			...
			if ( x > rect.left && x < rect.right && y > rect.top && y < rect.bottom ) {
				...
				dispatchPointerEquivalent( element, event, mouseEventInit );
