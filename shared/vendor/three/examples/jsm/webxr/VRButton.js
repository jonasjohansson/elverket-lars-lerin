		...
		function showEnterVR( /*device*/ ) {
			...
			async function onSessionStarted( session ) {
				...
				try {
					...
					// The WebXR API Emulator Chrome extension hands back a session object
					// that isn't a real XRSession, which trips three.js's XRWebGLBinding.
					// Swallow the noise, reset the button, and end the emulator session.
					console.warn( '[VRButton] setSession failed — emulator incompatibility?', err );
			...
			function onSessionEnded( event ) {
				...
				// currentSession may be null if our setSession catch triggered session.end().
				const s = currentSession || ( event && event.target );
				if ( s && s.removeEventListener ) s.removeEventListener( 'end', onSessionEnded );
