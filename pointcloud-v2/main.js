...
async function init() {
  ...
  // Cursor field uniforms
  const uCursorWorld = uniform(new THREE.Vector3(9999, 9999, 9999));
  ...
  material.positionNode = Fn(() => {
    ...
    // ── Cursor wither field: particles near cursor push outward + jitter, with slow chaotic drift ──
    const toCur = pos.sub(uCursorWorld);
    const curDist = toCur.length().add(0.001);
    // 1 at center, 0 past radius (invert smoothstep since it requires edge0 < edge1)
    const curFall = float(1.0).sub(smoothstep(uCursorRadius.mul(0.15), uCursorRadius, curDist)).mul(uCursorStrength);
    const curFall2 = curFall.mul(curFall); // sharper falloff for displacement
    const pushDir = toCur.div(curDist);
    const pushMag = curFall2.mul(uCursorPush);
    const cursorPushVec = pushDir.mul(pushMag);
    const ct = t.mul(0.6);
    ...
    return pos.add(vec3(
      w1x.add(w2x).add(w3x).add(dx).add(morphTurbX).add(windX).add(dissolveTurbX).add(curlX).add(transX).add(cursorPushVec.x).add(cx),
      w1y.add(w2y).add(w3y).add(dy).add(morphTurbY).add(dissolveTurbY).add(grav).add(lift).add(curlY).add(transY).add(cursorPushVec.y).add(cy),
      w1z.add(w2z).add(morphTurbZ).add(sin(t.mul(0.7).add(seedX.mul(seedY).mul(3.0))).mul(0.15).mul(uTurb).mul(d)).add(transZ).add(cursorPushVec.z).add(cz)
    ));
  })();
  ...
  material.colorNode = Fn(() => {
    ...
    // Cursor wither: fade and darken near cursor (same falloff as positionNode).
    const toCur = pos.sub(uCursorWorld);
    const curDist = toCur.length().add(0.001);
    const curFall = float(1.0).sub(smoothstep(uCursorRadius.mul(0.15), uCursorRadius, curDist)).mul(uCursorStrength);
    ...
    return vec4(witheredCol, uOpacity.mul(dissolveFade).mul(transitionFade).mul(cursorAlpha));
  })();
  ...
  info.textContent = `${(PARTICLE_COUNT / 1e6).toFixed(1)}M · WebGPU · Space: dissolve · T: transition · R: reset`;
  ...
  const fC = gui.addFolder('Cursor Wither');
  fC.add(params, 'cursorRadius', 0.1, 2.5, 0.05).name('Radius').onChange(v => uCursorRadius.value = v);
  fC.add(params, 'cursorPush', 0, 0.8, 0.01).name('Push').onChange(v => uCursorPush.value = v);
  ...
  function animate() {
    ...
    // Cursor field: lag the smoothed center behind the real pointer, ramp strength in/out
    cursorSmoothed.lerp(cursorTarget, params.cursorFollow);
    uCursorWorld.value.copy(cursorSmoothed);
