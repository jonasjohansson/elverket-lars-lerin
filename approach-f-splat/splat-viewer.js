...
void main () {
    ...
    // ─── Dissolution ───
    if (u_dissolve > 0.0) {
      float nx = (worldPos.x + 3.0) / 6.0; // normalize x roughly
      float randomDelay = fract(sin(float(index) * 12.9898) * 43758.5453) * 0.3;
      float activation = nx * 0.4 + randomDelay;
      float d = clamp((u_dissolve * 1.3 - activation) / (1.0 - activation + 0.001), 0.0, 1.0);
      float t = d * d * d;

      // Per-splat variation
      float windVar = 0.5 + fract(sin(float(index) * 7.31) * 1e4);
      float seed1 = fract(sin(float(index) * 3.17) * 1e4) * 6.28;
      float seed2 = fract(sin(float(index) * 5.43) * 1e4) * 6.28;

      // Wind + turbulence — strong enough to blow splats off screen
      worldPos.x -= t * 15.0 * windVar;
      worldPos.x += sin(u_time * 0.7 + seed1) * 2.0 * t;
      worldPos.y += sin(u_time * 1.3 + seed2) * 1.5 * t;
      worldPos.y -= t * 1.0 * windVar; // gravity
      worldPos.z += sin(u_time * 0.9 + seed1 + seed2) * 1.0 * t;
    }
    ...
    // Fade out dissolving splats
    if (u_dissolve > 0.0) {
      float nx = (uintBitsToFloat(cen.x) + 3.0) / 6.0;
      float rd = fract(sin(float(index) * 12.9898) * 43758.5453) * 0.3;
      float act = nx * 0.4 + rd;
      float dd = clamp((u_dissolve * 1.3 - act) / (1.0 - act + 0.001), 0.0, 1.0);
      vColor.a *= 1.0 - smoothstep(0.3, 1.0, dd);
    }
    vPosition = position;
...
async function main() {
    ...
    let dissolveValue = 0.0;
    let dissolveAnim = null;
    const startTime = performance.now() / 1000;

    // Keyboard controls: Space = play/reset dissolve, D = scrub with mouse
    window.addEventListener('keydown', (e) => {
      if (e.code === 'Space') {
        e.preventDefault();
        if (dissolveAnim) { dissolveValue = 0; dissolveAnim = null; }
        else {
          const t0 = performance.now();
          dissolveAnim = () => {
            dissolveValue = Math.min(1, (performance.now() - t0) / 8000);
            if (dissolveValue >= 1) dissolveAnim = null;
          };
        }
      }
      if (e.code === 'KeyR') { dissolveValue = 0; dissolveAnim = null; }
    });
    ...
    const frame = (now) => {
        ...
        if (vertexCount > 0) {
            ...
            if (dissolveAnim) dissolveAnim();
            gl.uniformMatrix4fv(u_view, false, actualViewMatrix);
            gl.uniform1f(u_dissolve, dissolveValue);
            gl.uniform1f(u_time_loc, performance.now() / 1000 - startTime);
